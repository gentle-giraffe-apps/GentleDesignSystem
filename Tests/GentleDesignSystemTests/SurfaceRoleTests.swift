...
// MARK: - GentleSurfaceDepthEffect Tests

@Suite("GentleSurfaceDepthEffect Tests")
struct GentleSurfaceDepthEffectTests {

    @Test("noEffect has correct properties")
    func testNoEffect() {
        let effect = GentleSurfaceDepthEffect.noEffect
        #expect(effect.displayName == "None")
        #expect(effect.strength == 0)
        #expect(effect.hasEffect == false)
    }

    @Test("highlightAndIndent has correct properties")
    func testHighlightAndIndent() {
        let effect = GentleSurfaceDepthEffect.highlightAndIndent(strength: 0.5)
        #expect(effect.displayName == "Highlight & Indent")
        #expect(effect.strength == 0.5)
        #expect(effect.hasEffect == true)
    }

    @Test("highlightAndIndent with zero strength still hasEffect")
    func testHighlightAndIndentZeroStrength() {
        let effect = GentleSurfaceDepthEffect.highlightAndIndent(strength: 0)
        #expect(effect.hasEffect == true)
        #expect(effect.strength == 0)
    }

    @Test("noEffect is codable")
    func testNoEffectCodable() throws {
        let original = GentleSurfaceDepthEffect.noEffect
        let encoder = JSONEncoder()
        let decoder = JSONDecoder()

        let data = try encoder.encode(original)
        let decoded = try decoder.decode(GentleSurfaceDepthEffect.self, from: data)

        #expect(decoded == original)
        #expect(decoded.hasEffect == false)
    }

    @Test("highlightAndIndent is codable")
    func testHighlightAndIndentCodable() throws {
        let original = GentleSurfaceDepthEffect.highlightAndIndent(strength: 0.75)
        let encoder = JSONEncoder()
        let decoder = JSONDecoder()
    ...
    @Test("highlightAndIndent decodes with default strength when missing")
    func testHighlightAndIndentDecodesWithDefaultStrength() throws {
        let json = """
        {"type": "highlightAndIndent"}
        """
        ...
        #expect(decoded.hasEffect == true)
        #expect(decoded.strength == 0.1) // Default strength
    }

    @Test("Surface depth effect equatable")
    func testSurfaceDepthEffectEquatable() {
        let noEffect1 = GentleSurfaceDepthEffect.noEffect
        let noEffect2 = GentleSurfaceDepthEffect.noEffect
        let highlight1 = GentleSurfaceDepthEffect.highlightAndIndent(strength: 0.5)
        let highlight2 = GentleSurfaceDepthEffect.highlightAndIndent(strength: 0.5)
        let highlight3 = GentleSurfaceDepthEffect.highlightAndIndent(strength: 0.3)

        #expect(noEffect1 == noEffect2)
        #expect(highlight1 == highlight2)
        #expect(highlight1 != highlight3)
        #expect(noEffect1 != highlight1)
    }
}
...
struct GentleSurfaceRoleSpecLegacyDecodingTests {
    ...
    @Test("Decodes legacy specularEffect key")
    func testDecodesLegacySpecularEffect() throws {
        let json = """
        {
            "backgroundStyle": {"type": "solid", "colorRole": "surfaceBase"},
            "specularEffect": {"type": "highlightAndIndent", "strength": 0.3},
            "border": {"lightHex": "#FFFFFF", "darkHex": "#000000"}
        }
        """
        let data = Data(json.utf8)
        ...
        #expect(decoded.surfaceDepthEffect.hasEffect == true)
        #expect(decoded.surfaceDepthEffect.strength == 0.3)
    }

    @Test("Decodes legacy specularStrength key")
    func testDecodesLegacySpecularStrength() throws {
        let json = """
        {
            "backgroundStyle": {"type": "solid", "colorRole": "surfaceBase"},
        ...
        #expect(decoded.surfaceDepthEffect.hasEffect == true)
        #expect(decoded.surfaceDepthEffect.strength == 0.4)
    }

    @Test("Defaults to noEffect when no depth effect specified")
    func testDefaultsToNoEffect() throws {
        let json = """
        {
            "backgroundStyle": {"type": "solid", "colorRole": "surfaceBase"},
            "border": {"lightHex": "#FFFFFF", "darkHex": "#000000"}
        ...
        #expect(decoded.surfaceDepthEffect == .noEffect)
    }
